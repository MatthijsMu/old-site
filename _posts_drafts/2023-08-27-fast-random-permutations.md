...
The second method relies on the fact that we can identify any $$N$$-permutation with its *inversion vector*. An inversion for $$\pi \in S_N$$ is a pair $$(i,j) \in \{1,...,N\}^2$$ such that $$i<j$$ but $$\pi(i)>\pi(j)$$. More specifically, we can count the number of inversions $$(i,j)$$ for one element $$i\in N$$, say $$\text{inv}(i)$$, and put these in a vector: $$(\text{inv}(1), ... , \text{inv}(N))$$. $$\text{inv}(i)$$ is a count of all $$j>i$$ with $$\pi(j)<\pi(i)$$. 
...
This gives us two methods of translating between direct representations and inversion vector representations. Note however that the method $$\text{inv}(\pi) \mapsto \pi$$ requires $$N - 1$$ interleavings, making it hideous to do in an ordinary array. 

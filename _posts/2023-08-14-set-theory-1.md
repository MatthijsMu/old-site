...
The notion of a set is something very intuitive that we use everyday. Yet a rigorous definition of a set is, when you first try this by yourself, quite difficult to give. There seems to be nothing more elementary than the set itself, so it is hard to find something "below the set" to describe a set with. 
---
